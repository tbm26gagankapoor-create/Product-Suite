...
export const onboardingService = {
  ...
  async createOrganizationWithOwner(input: OnboardingInput): Promise<OnboardingResult> {
    ...
    const user = {
      ...
      avatar_url: input.owner.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(input.owner.name)}`,
  ...
  async createUserUnderOrganization(
    ...
    const user = {
      ...
      email: userInput.email.toLowerCase(),
      password_hash: passwordHash,
      avatar_url: userInput.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(userInput.name)}`,
