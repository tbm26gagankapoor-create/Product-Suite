/**
 * Week 9: Comprehensive Testing Suite V2
 * Fixed to properly create test users and handle authentication
 */
...
async function setupTestUser() {
  ...
  await test('Setup', 'Verify user can authenticate', async () => {
    const response = await apiCall('GET', '/auth/me', undefined, userToken);
    if (response.status !== 200) throw new Error(`Status: ${response.status}`);
    if (!response.data.data?.email) throw new Error('No user data returned');

    // Get organization ID from user data
    testOrgId = response.data.data.organization_id || response.data.data.current_organization_id;
  });
}
...
async function testAuthentication() {
  ...
  await test('Auth', 'Reject protected endpoint with no token', async () => {
...
async function testSecurity() {
  ...
  await test('Security', 'Protected endpoints require authentication', async () => {
    const response = await apiCall('GET', '/projects');
    if (response.status === 200) throw new Error('Protected endpoint accessible without auth!');
  });
}
...
async function testTenantIsolation() {
  ...
  await test('Tenant', 'Projects scoped to organization', async () => {
    const response = await apiCall('GET', '/projects', undefined, userToken);
    if (response.status !== 200) throw new Error(`Status: ${response.status}`);
  ...
  await test('Tenant', 'Tasks scoped to organization', async () => {
    const response = await apiCall('GET', '/tasks', undefined, userToken);
    if (response.status !== 200) throw new Error(`Status: ${response.status}`);
...
async function testDataIntegrity() {
  ...
  await test('Data', 'All projects have required fields', async () => {
    ...
    const projects = response.data.data || [];
    for (const project of projects) {
      if (!project.id && !project._id) throw new Error('Project missing id');
  ...
  await test('Data', 'All tasks have required fields', async () => {
    ...
    const tasks = response.data.data || [];
    for (const task of tasks) {
      if (!task.id && !task._id) throw new Error('Task missing id');
...
async function runAllTests() {
  log('\n' + '='.repeat(60), 'bright');
  log('  Week 9: Comprehensive Testing Suite V2', 'bright');
