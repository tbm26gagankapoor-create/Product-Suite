...
import { generateTask } from '../lib/prompts';
...
const CreateTaskModal: React.FC<CreateTaskModalProps> = ({ 
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      const td = generateTask(aiPrompt, type, type === 'epic');
                                        ...
                                        {tempSubtasks.map((sub, idx) => (
                                            <div key={sub.id} className="flex items-center justify-between px-4 py-3 text-sm group hover:bg-gray-100 dark:hover:bg-[#1F2128] transition-colors">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-[#172B4D] dark:text-gray-900 dark:text-white font-medium">{sub.title}</span>
                                                </div>
                                                <button 
                                                    type="button" 
