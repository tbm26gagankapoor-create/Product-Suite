...
const PRDView: React.FC<PRDViewProps> = ({ project }) => {
    ...
    useEffect(() => {
        const loadGitSettings = async () => {
            if (project?.id) {
                try {
                    const settings = await gitService.getProjectGitSettings(project.id);
    ...
    const handleAiSubmit = async () => {
        ...
        try {
            ...
            const newContent = (response.text || '').replace(/```(?:markdown|md)?\n?([\s\S]*?)\n?```/gi, '$1').trim();
    ...
    const handleAddSection = () => {
        ...
        setLocalDocs(prev => ({ ...prev, [newId]: `## ${newSectionName}\n\nStart writing here...` }));
    ...
    return (
        <div className="flex h-full bg-white dark:bg-[#0B0C0E] text-[#172B4D] dark:text-gray-100 overflow-hidden relative transition-colors duration-200">
                        ...
                        <div className="max-w-4xl mx-auto p-8 sm:p-12 pb-48">
                            ...
                            <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200 dark:border-[#1F2128]">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    >
                                        {isEditing ? <><Eye size={14} /><span>Preview</span></> : <><Code2 size={14} /><span>Edit</span></>}
                                    </button>
                                    <button
                                    ...
                                    <button
                                        ...
                                        title={gitSettings?.enabled ? `Linked to ${gitSettings.repository.full_name}` : 'Link a Git repository'}
                                    >
                                        <GitBranch size={14} />
                            ...
                            <div className="relative min-h-[500px]">
                                ...
                                {isEditing ? (
                                    <textarea
                                        ...
                                        onChange={(e) => {
                                            setLocalDocs(prev => ({ ...prev, [activeSection]: e.target.value }));
                                            setHasUnsavedChanges(true);
                                        }}
                                        className={`w-full min-h-[500px] bg-gray-50 dark:bg-[#15171E] border border-gray-200 dark:border-[#2D2F36] rounded-lg p-6 text-sm font-mono text-[#172B4D] dark:text-gray-200 focus:outline-none focus:border-blue-500 resize-y ${isGenerating ? 'opacity-50' : 'opacity-100'} transition-opacity`}
                                        placeholder="Write markdown here..."
                                    />
                                ) : currentDocContent ? (
                                    <div className={`min-h-[400px] ${isGenerating ? 'opacity-50' : 'opacity-100'} transition-opacity`}>
                                    ...
                                    <div className="p-12 text-center text-gray-500 dark:text-gray-400 flex flex-col items-center justify-center min-h-[400px] select-none">
                                        ...
                                        <p>Click Edit or use AI to generate content.</p>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {/* Sync Docs Modal */}
            {showSyncDocsModal && project && gitSettings && (
                <SyncDocsModal
                    projectId={project.id}
                    projectName={project.name}
